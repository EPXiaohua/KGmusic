# MD3Music - Material Design 3 音乐播放器 - iOS移植版
...
- **AI 歌曲推荐** — 根据当前播放歌曲一键获取相似推荐，长按歌曲即可触发
- **歌手详情** — 歌手歌曲浏览，支持搜索/定位/排序，关注/取消关注歌手
- **歌曲评论** — 歌单/专辑/歌曲评论查看，支持按热度排序、楼中楼展开、分页加载与评论字号调节
...
- **嵌入封面与歌词** — 优先读取文件内嵌封面（ID3/APIC）与歌词（USLT/LYRICS），断网也能正常显示
...
- **文字阴影** — 背景图下改善可读性，阴影磅数可调（默认关闭）
- **显示大小** — 与系统同名设置一致：整体等比放大/缩小界面
