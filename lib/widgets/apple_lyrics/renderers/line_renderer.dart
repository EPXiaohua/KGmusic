...
class LineRenderer {
  ...
  /// 是否正处于"退场淡出"中（由 [beginExitFadeFrom] 置位，收敛后自动清除）。
  bool _isExitFading = false;

  /// 本次退场淡出的实际速率（/s）。
  ///
  /// 由 [beginExitFadeFrom] 的 [rate] 参数注入（按该行歌词时长动态计算，
  /// 快歌更快淡完、慢歌最多 1s），缺省回退到 [_exitFadeSpeed]。
  double _exitRate = _exitFadeSpeed;

  /// 退场淡出速度（缺省，按耗时 1s 反推）。
  ///
  /// 按「从退场初值衰减到 alphaEpsilon(0.001) 并吸附」耗时 1.0s 反推：
  /// α₀≈0.8 时 r = ln(0.8/0.001)/1.0 = ln(800) ≈ 6.7。
  /// 实际值由调用方按歌词行时长动态覆盖（见 [_exitRate]）。
  static const double _exitFadeSpeed = 6.7;
  ...
  /// v4 优化：上次 set text + layout 时的行高系数。
  ///
  /// 行间距（`LyricLayout.lineHeight`）变化时也必须重新 set text + layout：
  /// TextStyle.height 决定字形在行盒内的垂直位置，缓存命中会停留在旧行高上，
  /// 表现为"调行间距不刷新"。
  double _lastSetLineHeight = -1;
  ...
  /// 该行是否正在退场淡出。
  ///
  /// 模糊图已就绪时 painter 会跳过清晰层绘制（省电优化），但退场中的行
  /// 必须继续绘制，否则淡出被跳过、仍是硬切。
  bool get isExitFading => _isExitFading;
  ...
  /// 行退场交接：把清晰层 alpha 接续到 [value] 并启动淡出。
  ///
  /// [rate] 为该行的实际淡出速率（/s），按歌词行时长动态计算
  /// （时长 = min(行时长×40%, 1s)）；缺省用 [_exitFadeSpeed]。
  ///
  /// 必须显式调用。某行作为当前行期间走的是 [WordRenderer]，本实例那一帧
  /// 根本没被调用过，其 `setLineState` 输入缓存认为"isActive 未变"而早退，
  /// `_targetAlpha` 停留在构造初值 0.2、`_currentAlpha` 停留在 0 →
  ...
  void tick(double dt) {
    ...
    // 退场淡出走专用速率（可按行时长动态设置）；其余变亮用 ATTACK（快）、变暗用 RELEASE（慢）
  ...
  void paintLine(
    ...
    final bool lineHeightChanged = LyricLayout.lineHeight != _lastSetLineHeight;
    if (lineChanged ||
