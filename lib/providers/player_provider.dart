...
class PlayerProvider extends ChangeNotifier with WidgetsBindingObserver {
  ...
  // 上一次网络可用状态（用于感知「断网→恢复」转变）。默认 true：启动时假定有网络。
  bool _lastHasNetwork = true;
  ...
  // —— 在线歌曲 CDN 限速自动恢复 ——
  // 部分 Hi-Res 歌曲播放 ~60s 后 CDN 限速（网速从 MB/s 降至 KB/s），
  // 表现为进度条在 1:00~1:01 间反复弹跳、无声音（position 并非完全停滞，
  // 而是前进速率远低于 1x）。因此用"速率检测"：每 2 秒采样一次，
  // 若 position 前进量不足墙钟流逝的一半，判定卡死并刷新 URL 续播。
  // 检测延迟 ≈ 2 个采样窗口（约 4 秒），加上 API 刷新 ~1 秒，5 秒内恢复。
  // 预防性预取：position 到达 _stallPrefetchPositionSec 时后台预取新 URL，
  // 卡死瞬间直接用缓存 URL 续播（跳过 API 请求，等待趋近 0）。
  static const int _stallMinPositionSec = 45;
  ...
  int _stallRetries = 0;
  String? _stallSongId;
  DateTime _rateCheckTime = DateTime.now();
  Duration _rateCheckPos = Duration.zero;
  bool _rateCheckSeenPlay = false;
  // 预取的 CDN 播放 URL（卡死时直接使用，免 API 等待）
  String? _prefetchedCdnUrl;
  String? _prefetchedCdnSongId;
  String? _prefetchedCdnQuality;
  ...
  void _warnQualityDowngrade(String requested, String actual) {
    if (requested == actual || !_showQualityDowngradeToast) return;
    showToast(
  ...
  /// 同时感知「断网→恢复」转变：网络恢复时自动重试处于加载失败状态的当前歌曲
  ...
  void _onNetworkChanged(bool isWifi) {
    ...
    // 网络类型变化（含断网→恢复）时，若当前歌曲处于加载失败状态则自动重试
    _retryFailedPlayback();
  }

  /// 网络恢复或回到前台时，自动重试处于「加载失败」状态的当前歌曲。
  /// 仅当存在失败标记（[_resolveError] 非空）且未在播放/解析中时触发，
  /// 避免打扰用户主动暂停的播放。成功后停止后台兜底重试定时器。
  ...
  void _stopRetryFailedTimer() {
    _retryFailedTimer?.cancel();
  ...
  /// 设置「音质降级提示」开关：持久化 + 更新内存缓存（同步判断路径）。
  Future<void> setShowQualityDowngradeToast(bool value) async {
    _showQualityDowngradeToast = value;
    notifyListeners();
    try {
      await SettingsRepository().setShowQualityDowngradeToast(value);
    } catch (_) {}
  }
  ...
  Future<void> _restoreState() async {
    ...
    try {
      ...
      // 跳过恢复并清掉持久化状态，避免恢复出无法播放的「幽灵歌曲」
      if (shouldSkipRestoredSong(state.currentSong)) {
  ...
  /// 立即保存游标类播放状态（切歌、暂停、循环模式变更、位置防抖时调用）。
  ///
  ...
  void _initStreams() {
    ...
    try {
      _positionSubscription = _audioService.positionStream.listen((position) {
        ...
        // —— CDN 限速速率检测 + 预防性预取 ——
        // 每 _rateCheckInterval 采样一次 position，若前进量不足墙钟流逝的
        // _minPlaybackRate 倍（如弹跳在 1:00~1:01 之间、速率 ~0.1x），
        // 判定 CDN 限速卡死，刷新 URL 从断点续播。
        // 保护：仅在线歌曲、position >= 45s、播放中、非加载保护期。
        if (_isPlaying &&
            !insideLoadGuard &&
            !_handlingCompletion &&
            _currentSong?.isOnline == true &&
            position.inSeconds >= _stallMinPositionSec) {
          final now = DateTime.now();
          final elapsed = now.difference(_rateCheckTime);
          if (elapsed >= _rateCheckInterval) {
            final advanced = position - _rateCheckPos;
            ...
            _rateCheckTime = now;
            _rateCheckSeenPlay = true;
          }
          // 预防性预取：position 接近限速点（~52s）时后台预取新 URL，
          // 卡死瞬间直接使用，免 API 请求等待。
          if (!_prefetchInFlight &&
              _prefetchedCdnSongId != _currentSong?.id &&
              position.inSeconds >= _stallPrefetchPositionSec) {
            _prefetchCdnUrl();
          }
        }
  ...
  /// 后台预取 CDN 播放 URL，供卡死时直接使用（免 API 等待）。
  /// 仅对在线歌曲生效；position 到达 _stallPrefetchPositionSec 时由
  /// positionStream 触发一次（_prefetchedCdnSongId 防止重复请求）。
  Future<void> _prefetchCdnUrl() async {
    final song = _currentSong;
    if (song == null || !song.isOnline) return;
    ...
    try {
      ...
      if (result != null &&
        ...
        _prefetchedCdnUrl = result.url;
        _prefetchedCdnSongId = song.id;
        _prefetchedCdnQuality = result.quality;
        debugPrint('[CdnPrefetch] 预取 URL 成功 (${result.quality})');
      }
    } catch (e) {
      debugPrint('[CdnPrefetch] 预取失败: $e');
    } finally {
      _prefetchInFlight = false;
    }
  }

  /// CDN 限速导致播放位置停滞时，刷新 URL 从断点续播。
  ...
  Future<void> _handleCdnStall() async {
    ...
    if (_stallSongId != song.id) {
      _stallSongId = song.id;
      _stallRetries = 0;
    }
    _stallRetries++;
    ...
    try {
      ...
      String? newUrl;
      String? newQuality;
      if (_prefetchedCdnSongId == song.id && _prefetchedCdnUrl != null) {
        newUrl = _prefetchedCdnUrl;
        newQuality = _prefetchedCdnQuality;
        debugPrint('[CdnStall] 使用预取 URL');
      } else {
        final result = await KugouApiClient().getSongUrlWithFallback(
          song.id,
          quality: _audioQuality.value,
  ...
  Future<void> _downgradeOnStall(Song song) async {
    ...
    switch (currentQuality) {
      case KugouQuality.hires:
        targetQuality = KugouQuality.lossless;
      case KugouQuality.lossless:
        targetQuality = KugouQuality.high;
      default:
        targetQuality = null;
    }
    if (targetQuality == null) {
      debugPrint('[CdnStall] 当前音质($currentQuality)已无可降级档位，放弃');
      return;
    }
  ...
  void setSleepTimer(Duration? duration) {
    ...
    _scheduleSleepNotifier(duration);
    _sleepTimerTicker = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      if (left <= Duration.zero) {
        ...
        // 值以 post-frame 落地：与可能同时进行的 route 切换（切歌 pop/重建）
        // 解耦，避免 InheritedElement 的 _dependents 断言竞态。
  ...
  /// 延迟一帧更新睡眠定时剩余时间 notifier。
  ///
  /// 直接同步赋值会在调用者（bottom sheet onTap 打开/关闭路由）的同步帧内
  /// 触发 ValueListenableBuilder 重建其子树；若该帧恰逢 ModalScope/Theme 等
  /// InheritedElement 被 pop/deactivate，会触发 Flutter 的
  /// `InheritedElement._dependents.isEmpty` 断言。改用 post-frame 回调，
  /// 把通知与路由生命周期帧解耦。
...
class AudioServiceLoader {
  static Future<dynamic> Function()? _testOverride;

  /// 测试入口：注入替代加载器（传 null 清空），避免宿主环境实际构造
  /// just_audio（Windows 上加载平台实现可能挂起）。生产环境恒为 null。
  @visibleForTesting
  static void setTestOverride(Future<dynamic> Function()? loader) {
    _testOverride = loader;
  }
...
/// 判断持久化状态恢复出的歌曲是否应跳过。
///
/// 只针对外部调用歌曲（id 前缀 `external_`）：其 `localPath` 指向私有缓存拷贝，
/// 文件可能已被缓存清理（见维护要点）或系统清空，恢复会得到无法播放的
/// 「幽灵歌曲」。在线歌曲的 URL 过期、普通本地歌曲被删除均为既有容忍场景，
/// 保持原行为不扩大变更面。
bool shouldSkipRestoredSong(Song song) {
  if (song.isOnline) return false;
  if (!song.id.startsWith('external_')) return false;
